...
function pmprodon_pmpro_membership_level_after_other_settings() {
	...
	?>
<h3 class="topborder"><?php esc_html_e( 'Donations', 'pmprodon' ); ?></h3>
<p><?php esc_html_e( 'If donations are enabled, users will be able to set an additional donation amount at checkout. That price will be added to any initial payment you set on this level. You can set the minimum and maxium amount allowed for gifts for this level.', 'pmprodon' ); ?></p>
<table>
<tbody class="form-table">
	<tr>
		<th scope="row" valign="top"><label for="donations"><?php esc_html_e( 'Enable:', 'pmprodon' ); ?></label></th>
		<td>
			<input type="checkbox" id="donations" name="donations" value="1" <?php checked( $donations, '1' ); ?> /> <label for="donations"><?php esc_html_e( 'Enable Donations', 'pmprodon' ); ?></label>
		</td>
	</tr>
	<tr>
		<th scope="row" valign="top"><label for="donation_min_price"><?php esc_html_e( 'Min Amount:', 'pmprodon' ); ?></label></th>
		<td>
	...
	<tr>
		<th scope="row" valign="top"><label for="donation_max_price"><?php esc_html_e( 'Max Amount:', 'pmprodon' ); ?></label></th>
		<td>
	...
	<tr>
		<th scope="row" valign="top"><label for="dropdown_prices"><?php esc_html_e( 'Price Dropdown:', 'pmprodon' ); ?></label></th>
		<td>
			<input type="text" id="dropdown_prices" name="dropdown_prices" size="60" value="<?php echo esc_attr( $dropdown_prices ); ?>" /><br /><small><?php esc_html_e( "Enter numbers separated by commas to popuplate a dropdown with suggested prices. Include 'other' (all lowercase) in the list to allow users to enter their own amount.", 'pmprodon' ); ?></small>
		</td>
	</tr>
	<tr>
		<th scope="row" valign="top"><label for="donations_text"><?php esc_html_e( 'Help Text:', 'pmprodon' ); ?></label></th>
		<td>
			<textarea id="donations_text" name="donations_text" rows="5" cols="60"><?php echo esc_textarea( $donations_text ); ?></textarea>
			<br /><small><?php esc_html_e( 'If not blank, this text will override the default text generated to explain the range of donation values accepted.', 'pmprodon' ); ?></small>
		</td>
	</tr>
</tbody>
</table>
...
function pmprodon_pmpro_checkout_after_level_cost() {
	...
	if ( $pmpro_review ) {
		?>
		<input type="hidden" name="donation" value="<?php echo esc_attr( $donation ); ?>" /><?php } ?>
	</span>
	<?php
	if ( empty( $pmpro_review ) ) {
		...
		if ( ! empty( $donfields['text'] ) ) {
			...
			printf( __( 'Enter an amount between %1$s and %2$s', 'pmprodon' ), pmpro_formatPrice( $donfields['min_price'] ), pmpro_formatPrice( $donfields['max_price'] ) );
		}
		?>
			</p>
			<?php
...
function pmprodon_pmpro_invoice_bullets_bottom( $order ) {
	...
	if ( ! empty( $components['donation'] ) ) {
		?>
	<li><strong><?php esc_html_e( 'Membership Cost', 'pmprodon' ); ?>: </strong> <?php echo pmpro_formatPrice( $components['price'] ); ?></li>
	<li><strong><?php esc_html_e( 'Donation', 'pmprodon' ); ?>: </strong> <?php echo pmpro_formatPrice( $components['donation'] ); ?></li>
		<?php
	}
}
...
function pmprodon_plugin_row_meta( $links, $file ) {
	if ( strpos( $file, 'pmpro-donations.php' ) !== false ) {
		$new_links = array(
			...
			'<a href="' . esc_url( 'https://paidmembershipspro.com/support/' ) . '" title="' . esc_attr( __( 'Visit Customer Support Forum', 'pmprodon' ) ) . '">' . __( 'Support', 'pmprodon' ) . '</a>',
		);
